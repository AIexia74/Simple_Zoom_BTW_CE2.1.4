...
public abstract class MinecraftServer implements ICommandSender, Runnable, IPlayerUsage
{
            ...
            if (this.startServer())
            {
                ...
                for (long timeSinceLastTick = 0L; this.serverRunning; this.serverIsRunning = true)
                {
                    ...
                    if (speedModifier > 1.0F) {
                        if (timeSinceLastTick > 50L / speedModifier)
                        {
                            timeSinceLastTick -= (long) (50L / speedModifier);
                            this.tick();
                    ...
                    if (currentTimeMillis - lastTimerUpdateTime > 1000L || sendTimerSpeedImmediately) {
                        speedModifier = this.getMinSpeedModifier();
                        this.sendTimerSpeedUpdate(
                            Math.min(
                                50F / ((float) (currentTimeMillis - lastTimerUpdateTime) / ticksSinceLastTimerUpdate),
                                speedModifier
                            )
                        );
                        lastTimerUpdateTime = System.currentTimeMillis();;
                        ticksSinceLastTimerUpdate = 0;
                        sendTimerSpeedImmediately = false;
                    }
    ...
    public void sendTimerSpeedImmediately() {
        sendTimerSpeedImmediately = true;
    }
