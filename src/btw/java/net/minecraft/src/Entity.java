...
public abstract class Entity
{
    ...
    public Entity(World par1World)
    {
        ...
        this.nextBlockCheckDistance = 0.5F;
        this.ySize = 0.0F;
    ...
    {
    	return riddenByEntity != null && riddenByEntity instanceof SquidEntity;
    }
    ...
    public void flingAwayFromEntity(Entity repulsingEntity, double dForceMultiplier)
	{
		...
		motionX = MathUtils.clampDouble(dVelocityX, -1D, 1D);
		motionY = MathUtils.clampDoubleTop(dVelocityY, 0.75D);
		motionZ = MathUtils.clampDouble(dVelocityZ, -1D, 1D);
    }
    ...
    public void moveEntity( double dMoveX, double dMoveY, double dMoveZ )
    {
        ...
        if ( canTriggerWalking() && !bIsSneakingPlayer && ridingEntity == null )
        {
            ...
            if (distanceWalkedOnStepModified > nextStepDistance && iGroundBlockID > 0) {
                ...
                if (isInWater()) {
                    float fSwimSoundVolume = MathHelper
                            .sqrt_double(motionX * motionX * 0.20000000298023224D + motionY * motionY + motionZ * motionZ * 0.20000000298023224D) * 0.35F;
            
                    if (fSwimSoundVolume > 1F) {
                        fSwimSoundVolume = 1F;
                    }
            
                    playSound("liquid.swim", fSwimSoundVolume, 1F + (rand.nextFloat() - rand.nextFloat()) * 0.4F);
                }
        
                playStepSound(iGroundI, iGroundJ, iGroundK, iGroundBlockID);
            }
            
            if (distanceWalkedOnStepModified > nextBlockCheckDistance && iGroundBlockID > 0) {
                nextBlockCheckDistance = distanceWalkedOnStepModified + 0.5F;
        
                if (lastGroundPosX != iGroundI || lastGroundPosY != iGroundJ || lastGroundPosZ != iGroundK) {
                    lastGroundPosX = iGroundI;
                    lastGroundPosY = iGroundJ;
                    lastGroundPosZ = iGroundK;
            
                    int feetBlockID = worldObj.getBlockId(iGroundI, iGroundJ + 1, iGroundK);
                    if (feetBlockID > 0) {
                        if (Block.blocksList[feetBlockID].groundCoverRestingOnVisualOffset(worldObj, iGroundI, iGroundJ + 1, iGroundK) < 0.0F) {
                            int groundCoverBlockID = worldObj.getBlockId(iGroundI, iGroundJ + 2, iGroundK);
                    
                            if (groundCoverBlockID > 0) {
                                Block.blocksList[groundCoverBlockID].onEntityStepsIn(worldObj, iGroundI, iGroundJ + 2, iGroundK, this);
                            }
                        }
                
                        Block.blocksList[feetBlockID].onEntityStepsIn(worldObj, iGroundI, iGroundJ + 1, iGroundK, this);
                    }
            
                    Block.blocksList[iGroundBlockID].onEntityWalking(worldObj, iGroundI, iGroundJ, iGroundK, this);
                }
            }
        }
