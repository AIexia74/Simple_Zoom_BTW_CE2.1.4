...
public class WorldInfo
{
    ...
    public WorldInfo(WorldSettings par1WorldSettings, String par2Str, boolean initGlobalData)
    {
        ...
        this.difficulty = par1WorldSettings.getDifficulty();
        
        if (initGlobalData) {
            createDefaultModInfo();
        }
    ...
    // FCMOD: Code added
    private InventoryEnderChest globalEnderChestInventory = new InventoryEnderChest();
    
    public boolean previouslyRaining = false;
    public boolean previouslyThundering = false;
    
    private boolean hasNetherBeenAccessed = false;
    private boolean hasWitherBeenSummoned = false;
    private boolean hasEndDimensionBeenAccessed = false;
    
    private Difficulty difficulty;

    public InventoryEnderChest getGlobalEnderChestInventory()
    {
    	return globalEnderChestInventory;
    }

    public void setGlobalEnderChestInventory(InventoryEnderChest inventory)
    {
        globalEnderChestInventory = inventory;
    }
    ...
    private void createDefaultModInfo() {
        for (Map.Entry<Class<? extends BTWAddon>, WorldData> entry : AddonHandler.addonWorldDataMap.entrySet()) {
            entry.getValue().createDefaultGlobalData(this);
        }
    }
