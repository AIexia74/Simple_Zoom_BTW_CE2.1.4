...
public class WorldServer extends World
{
	...
	public void tick()
	{
		...
		// send sync packets to all nearby players every 10 seconds
		if(getTotalWorldTime()%200 == 0){
			for(int i=0; i<playerEntities.size(); i++){
				EntityPlayer playerEntity = (EntityPlayer)playerEntities.get(i);
				this.mcServer.getConfigurationManager().sendToAllNearExcept(
						playerEntity,
	...
	protected void tickBlocksAndAmbiance()
	{
		...
		int var1 = 0;
		int var2 = 0;
		Iterator<ChunkCoordIntPair> var3 = activeChunksCoordsList.iterator();
		...
		while (var3.hasNext())
		{
			...
			if ( rand.nextInt( 50000 ) == 0 && isRaining() && isThundering() )
			{
				...
				if ( canLightningStrikeAtPos(var9, var11, var10) )
				{
					...
					BlockPos adjustedStrikePos = getAdjustedLightningStrikeLocation(strikePos);
		
					if ( isBlockPosActive(adjustedStrikePos.x, adjustedStrikePos.y, adjustedStrikePos.z) )
					{
						addWeatherEffect(EntityList.createEntityOfType(LightningBoltEntity.class, this, adjustedStrikePos.x + 0.5D, adjustedStrikePos.y, adjustedStrikePos.z + 0.5D));
					}
				}
			}
	...
	public void updateEntityWithOptionalForce(Entity par1Entity, boolean par2)
	{
		...
		if (!(par1Entity.riddenByEntity instanceof EntityPlayer))
		{
			...
			if (par2 && !isBlockPosActive(iEntityI, 0, iEntityK) && par1Entity.addedToChunk && !(par1Entity instanceof EntityPlayer))
			{
	...
	private long noPlayersOnServerTickCount = 0;
	...
	{
		chunksToCheckForUnloadList.add(new ChunkCoordIntPair(iChunkX, iChunkZ ));
	}
	...
	{
		if ( !chunksToCheckForUnloadList.isEmpty() )
		{
			Iterator<ChunkCoordIntPair> tempIterator = chunksToCheckForUnloadList.iterator();
			...
			chunksToCheckForUnloadList.clear();
		}
	}
	...
	private boolean checkChunkShouldBeUnloaded(int iChunkX, int iChunkZ)
	{
		return chunkExists( iChunkX, iChunkZ ) &&
			   !chunkTracker.isChunkBeingWatched(iChunkX, iChunkZ) &&
	...
	private void markChunksAroundSpawnToCheckForUnload()
	{
		...
		if ( provider.canRespawnHere() )
		{
			...
			addChunkRangeToCheckForUnloadList(
					iSpawnChunkX - chunksAroundSpawnToCheckForUnload,
					iSpawnChunkZ - chunksAroundSpawnToCheckForUnload,
					iSpawnChunkX + chunksAroundSpawnToCheckForUnload,
					iSpawnChunkZ + chunksAroundSpawnToCheckForUnload);
		}
	}
	...
	protected void updateWeather()
	{
		...
		if (worldInfo.previouslyRaining != worldInfo.isRaining() )
		{
			...
			worldInfo.previouslyRaining = worldInfo.isRaining();
		}

		if (worldInfo.previouslyThundering != worldInfo.isThundering() )
		{
			...
			worldInfo.previouslyThundering = worldInfo.isThundering();
		}
	}
	...
	private void validateMagneticPointList()
	{
		...
		if ( ( iTimeFactor & 15 ) == 0 )
		{
			int iListLength = magneticPointList.magneticPoints.size();
		...
		MagneticPoint tempPoint = (MagneticPoint) magneticPointList.magneticPoints.get(iTempIndex);

		if ( checkChunksExist(tempPoint.posX, 0, tempPoint.posZ, tempPoint.posX, 0, tempPoint.posZ) )
		{
			// just check if the point has an associated beacon.  If it does, the beacon itself can validate its precise state

			if (getBlockId(tempPoint.posX, tempPoint.posY, tempPoint.posZ) != Block.beacon.blockID )
			{
				magneticPointList.magneticPoints.remove(iTempIndex);
	...
	private void validateAmbientBeaconList() {
		// periodically check the ambient beacon list for dead points
		
		int timeFactor = (int) getWorldTime();
		
		if ((timeFactor & 15) == 0) {
			int listLength = ambientBeaconLocationList.effectLocations.size();
			...
			if (listLength > 0) {
				...
				BeaconEffectLocation tempPoint = (BeaconEffectLocation) ambientBeaconLocationList.effectLocations.get(i);
				
				if (checkChunksExist(tempPoint.posX, 0, tempPoint.posZ, tempPoint.posX, 0, tempPoint.posZ)) {
					// just check if the point has an associated beacon.  If it does, the beacon itself can validate its precise state
					
					if (getBlockId(tempPoint.posX, tempPoint.posY, tempPoint.posZ) != Block.beacon.blockID) {
						ambientBeaconLocationList.effectLocations.remove(i);
	...
	private void validateSpawnLocationList()
	{
		...
		if ( ( lWorldTime & 15 ) == 0 )
		{
			Iterator tempIterator = spawnLocationList.spawnLocations.iterator();

			while ( tempIterator.hasNext() )
			{
				SpawnLocation tempPoint = (SpawnLocation)tempIterator.next();

				if ( lWorldTime < tempPoint.spawnTime || lWorldTime - tempPoint.spawnTime > HardcoreSpawnUtils.HARDCORE_SPAWN_TIME_BETWEEN_REASSIGNMENTS)
				{        			
					tempIterator.remove();
				}        		
			}
	...
	protected BlockPos getAdjustedLightningStrikeLocation(BlockPos strikePos) {
		// Checks nearby entities to see if any are higher than the highest block
		BlockPos entityCheckPos = getHighestEntityForLightningStrike(strikePos, 16).orElse(strikePos);
		
		// Initial redirection check
		BlockPos firstBlockPassPos = getHighestPointForLightningStrike(entityCheckPos, 16, false).orElse(entityCheckPos);
		
		// Checks block location a second time to ensure that locations protected by pillars are guaranteed safe within the second range
		// This second pass is done after entities to make sure that blocks are able to protect entities
		BlockPos secondBlockPassPos = getHighestPointForLightningStrike(firstBlockPassPos, 16, false).orElse(firstBlockPassPos);
		
		// Finally, checks for lightning rods in an even larger radius
		return getHighestPointForLightningStrike(secondBlockPassPos, 32, BTWBlocks.lightningRod.blockID, true).orElse(secondBlockPassPos);
	}
	 ...
	 * @param strikePos
	 * @param radius
	 * @param blockIDs Set containing ids to check. Empty set instead looks for any block
	 * @return The position found, or empty if nothing matched the given blockIDs
	 */
	protected Optional<BlockPos> getHighestPointForLightningStrike(BlockPos strikePos, int radius, Set<Integer> blockIDs, boolean ignoreBiomeCheck) {
		...
		for (int i = minX; i <= maxX; i++) {
			for (int k = minZ; k <= maxZ; k++) {
				int j = getPrecipitationHeight(i, k);
				
				if (j > strikeY && (this.canLightningStrikeAtPos(i, j, k) || ignoreBiomeCheck)) {
					if (blockIDs.isEmpty() || blockIDs.contains(this.getBlockId(i, j - 1, k))) {
						strikeX = i;
						strikeY = j;
						strikeZ = k;
		...
		if (foundValidStrikeLocation) {
			return Optional.of(new BlockPos(strikeX, strikeY, strikeZ));
		}
		else {
	 ...
	 * @param strikePos
	 * @param radius
	 * @return The position found, or empty if no entities were higher than the starting point
	 */
	protected Optional<BlockPos> getHighestEntityForLightningStrike(BlockPos strikePos, int radius) {
		...
		List<Entity> entityList = getEntitiesWithinAABB(Entity.class, AxisAlignedBB.getAABBPool().getAABB(minX, strikeY, minZ, maxX + 1, 256, maxZ + 1));
		...
		for (Entity entity : entityList) {
			if (entity.isEntityAlive() && entity.attractsLightning()) {
				...
				if (entityMaxY > strikeY) {
					...
					int blockHeightAtEntity = getPrecipitationHeight(entityX, entityZ);
					
					if (blockHeightAtEntity <= entityMaxY && canLightningStrikeAtPos(entityX, blockHeightAtEntity, entityZ)) {
						strikeX = entityX;
	...
	protected void updateServerIdleState()
	{
		...
		{
			noPlayersOnServerTickCount++;
		}
		else
		{
			noPlayersOnServerTickCount = 0;
		}
	}

	protected boolean isServerIdle()
	{
		return noPlayersOnServerTickCount >= 1200L;
	}
	...
	public void saveWorldDataToNBT(File dimensionDirectory) {
		for (Map.Entry<Class<? extends BTWAddon>, WorldData> entry : AddonHandler.addonWorldDataMap.entrySet()) {
