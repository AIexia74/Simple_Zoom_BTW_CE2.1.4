...
public abstract class EntityLiving extends Entity
{
    ...
    public boolean hasAttackTarget() {
        return dataWatcher.getWatchableObjectByte(HAS_ATTACK_TARGET_DATA_WATCHER_ID) == 1;
    }
    ...
    protected void entityLivingSetAttackTarget(EntityLiving par1EntityLiving) {
        ...
        if (par1EntityLiving != null) {
            this.dataWatcher.updateObject(HAS_ATTACK_TARGET_DATA_WATCHER_ID, Byte.valueOf((byte) 1));
        }
        else {
            this.dataWatcher.updateObject(HAS_ATTACK_TARGET_DATA_WATCHER_ID, Byte.valueOf((byte) 0));
        }
    }
    ...
    protected void entityInit()
    {
        ...
        this.dataWatcher.addObject(HAS_ATTACK_TARGET_DATA_WATCHER_ID, Byte.valueOf((byte)0));
    }
    ...
    public boolean attackEntityFrom(DamageSource par1DamageSource, int par2)
    {
        ...
        else
        {
            ...
            else
            {
        		...
        		{
                    recentlyOnChoppingBlockCountdown = onChoppingBlockMaxCountdown;
            	}
            	// END FCMOD
    ...
    */
    ...
    {
    	if (recentlyOnChoppingBlockCountdown > 0 )
    	{
    ...
    public float getSpeedModifier()
    {
        ...
        if (onGround && isAffectedByMovementModifiers() )
        {
        	...
        	if ( WorldUtils.isGroundCoverOnBlock(worldObj, iGroundI, iGroundJ, iGroundK) )
        	{
        		fMoveSpeed *= 0.8F;
        	}
    ...
    protected void checkForHeadDrop(DamageSource source, int iLootingModifier) {
        ...
        // quadruple chance if killed by battle axe or chopping block
        if (sourceEntity instanceof EntityPlayer) {
        ...
        else if (source == CustomDamageSource.damageSourceChoppingBlock || recentlyOnChoppingBlockCountdown > 0) {
            iHeadChance = iHeadChance >> 2;
        }
        
        // further chance depending on looting (double at looting 4)
    ...
    protected int getAmbientLootingModifier()
    {
    	...
    	return worldObj.getAmbientBeaconEffectAtLocation(BeaconTileEntity.LOOTING_EFFECT.EFFECT_NAME, iLocI, iLocJ, iLocK);
    }
