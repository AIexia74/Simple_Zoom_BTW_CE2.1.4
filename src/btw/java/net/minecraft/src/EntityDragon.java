...
public class EntityDragon extends EntityLiving implements IBossDisplayData, IEntityMultiPart
{
    ...
    private void setNewTarget()
    {
        ...
        if (this.rand.nextInt(2) == 0 && !this.worldObj.playerEntities.isEmpty())
        {
            ...
            // FCMOD: Code added
            long lTargetChangedDimensionTime = ((EntityPlayer)target).timeOfLastDimensionSwitch;
            long lWorldTime = worldObj.getWorldTime();
            
            if ( lWorldTime < lTargetChangedDimensionTime || lWorldTime - lTargetChangedDimensionTime > PLAYER_SWITCH_DIMENSIONS_GRACE_PERIOD)
            {
