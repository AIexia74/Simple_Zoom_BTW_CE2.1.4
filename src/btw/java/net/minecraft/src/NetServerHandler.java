...
public class NetServerHandler extends NetHandler
{
    ...
    public void handleUseEntity(Packet7UseEntity par1Packet7UseEntity)
    {
        ...
        if (var3 != null)
        {
        	...
        	if ( var3 instanceof EntityWithCustomPacket)
        	{
        		if ( ((EntityWithCustomPacket)var3).shouldServerTreatAsOversized() )
        		{
	        		var4 = true;
	        		var5 = 256D;
        		}
    ...
    public void handleClientInfo(Packet204ClientInfo par1Packet204ClientInfo)
    {
        ...
        if ( !par1Packet204ClientInfo.isBTWInstalled() )
        {
	    	...
	    	playerEntity.playerNetServerHandler.sendPacketToPlayer( new Packet3Chat( (new StringBuilder()).append( "\2474"). // red text
	    		...
	    		append( BTWMod.instance.getVersionString()).toString() ) );
    ...
    public void handleStartBlockHarvest(StartBlockHarvestPacket packet)
    {
        ...
        if ( dDistSq <= 36D && j < mcServer.getBuildLimit() )
        {
	        if ( !isBlockWithinSpawnProtection(world, i, j, k) )
	        {
	            ...
	            playerEntity.theItemInWorldManager.onBlockClicked(i, j, k, packet.face);
	        }
	        else
	        {
	        	// I assume the following resets the block harvest on the player's client

	        	WorldUtils.sendPacketToPlayer(playerEntity.playerNetServerHandler, new Packet53BlockChange(i, j, k, world ));
	        }
        }
    }
    ...
    public void handlePlayerSync(PlayerSyncPacket packet){
        for(Object other : playerEntity.worldObj.playerEntities){
            EntityPlayerMP otherPlayer = (EntityPlayerMP) other;
            if(otherPlayer.username.equals(packet.playerName)){
                sendPacketToPlayer(new Packet20NamedEntitySpawn(otherPlayer));
                WorldServer worldServer = this.mcServer.worldServerForDimension(this.playerEntity.dimension);
                
                Iterator entryIterator = worldServer.getEntityTracker().trackedEntities.iterator();
                while (entryIterator.hasNext()) {
                    EntityTrackerEntry entry = (EntityTrackerEntry)entryIterator.next();
                    
                    if (entry.myEntity != playerEntity) {
                        entry.tryStartWachingThis(playerEntity);
                    }
                }
            }
        }
