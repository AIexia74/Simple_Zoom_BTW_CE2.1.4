...
public abstract class World implements IBlockAccess
{
    ...
    public void updateEntityWithOptionalForce(Entity par1Entity, boolean par2)
    {
        ...
        int var5 = LOADED_CHUNKS_UPDATE_RANGE;


    	// FCCHUNK: Decide on updates around original spawn
        if (!par2 || isBlockPosActive(var3, 0, var4) || this.isRemote)
        {
    ...
    public boolean canSnowAt(int x, int y, int z) {
        ...
        if (biome.canSnowAt(this, x, y, z)) {
            if (y >= 0 && y < 256 && this.getSavedLightValue(EnumSkyBlock.Block, x, y, z) < 10) {
                if (SnowCoverBlock.canSnowCoverReplaceBlock(this, x, y, z) && Block.snow.canPlaceBlockAt(this, x, y, z)) {
                    return true;
                }
            }
    ...
    public boolean canPlaceEntityOnSide(int par1, int par2, int par3, int par4, boolean par5, int par6, Entity par7Entity, ItemStack par8ItemStack)
    {
        ...
        else
        {
            if (var10 != null && (var10 == Block.waterMoving || var10 == Block.waterStill || var10 == Block.lavaMoving || var10 == Block.lavaStill || var10 == Block.fire || var10.blockMaterial.isReplaceable() ||
            	var10 == BTWBlocks.detectorLogic || var10 == BTWBlocks.glowingDetectorLogic ||
    ...
    {
    	if ( !canLightningStrikeAtMessageSent)
    	{
	    	BTWMod.debugWarning( (new StringBuilder()).
            ...
            canLightningStrikeAtMessageSent = true;
    	}
    ...
    private long timeOfLastSquidPossessionCapCount = -1;
    ...
    public Entity getClosestEntityMatchingCriteriaWithinRange(double dSourcePosX, double dSourcePosY, double dSourcePosZ, double dRange, ClosestEntitySelectionCriteria criteria)
    {
    	...
    	ClosestEntityInfo closestEntityInfo =
    		new ClosestEntityInfo( dSourcePosX, dSourcePosY, dSourcePosZ, dRange * dRange, null, criteria, iChunkEntityListMinVerticalIndex, iChunkEntityListMaxVerticalIndex );
    ...
    public int getNumEntitiesThatApplyToSquidPossessionCap()
    {
    	...
    	if (lCurrentTime != timeOfLastSquidPossessionCapCount)
    	{
            lastSquidPossessionCapCount = 0;
            timeOfLastSquidPossessionCapCount = lCurrentTime;
            ...
            for ( int iTempEntityIndex = 0; iTempEntityIndex < this.loadedEntityList.size(); ++iTempEntityIndex )
            {
                ...
                {
                	lastSquidPossessionCapCount++;
                }
            }            
    	}
    	
		return lastSquidPossessionCapCount;
    }

	public void notifyNearbyAnimalsOfPlayerBlockAddOrRemove(EntityPlayer player, Block block, int i, int j, int k)
	{
		if ( !isRemote && block.startlesAnimalsWhenPlaced(this, i, j, k) && !player.capabilities.isCreativeMode && this.getDifficulty().shouldBlocksStartleAnimals())
		{
    ...
    {
        return getBlockNaturalLightOnlyValueDo(i, j, k, true, skylightSubtracted);
    }

    public int getBlockNaturalLightValueMaximum(int i, int j, int k)
    {
        return getBlockNaturalLightOnlyValueDo(i, j, k, true, 0);
    }
    ...
    private int getBlockNaturalLightOnlyValueDo(int i, int j, int k, boolean bConsiderNeighbors, int iSkylightToSubtract)
    {
        ...
        if ( i >= -30000000 && k >= -30000000 && i < 30000000 && k < 30000000 )
        {
            if ( bConsiderNeighbors )
            {
                ...
                {
                    int iNeighbor1 = getBlockNaturalLightOnlyValueDo(i, j + 1, k, false, iSkylightToSubtract);
                    int iNeighbor2 = getBlockNaturalLightOnlyValueDo(i + 1, j, k, false, iSkylightToSubtract);
                    int iNeighbor3 = getBlockNaturalLightOnlyValueDo(i - 1, j, k, false, iSkylightToSubtract);
                    int iNeighbor4 = getBlockNaturalLightOnlyValueDo(i, j, k + 1, false, iSkylightToSubtract);
                    int iNeighbor5 = getBlockNaturalLightOnlyValueDo(i, j, k - 1, false, iSkylightToSubtract);
    ...
    {
    	return localEnderChestInventory;
    }
    
    public InventoryEnderChest getLocalLowPowerEnderChestInventory()
    {
    	return localLowPowerEnderChestInventory;
    }
    ...
    {
    	return ambientBeaconLocationList;
    }
    
    public int getAmbientBeaconEffectAtLocation(String effectName, int locX, int locY, int locZ)
    {
    	return ambientBeaconLocationList.getMostPowerfulBeaconEffectForLocation(effectName, locX, locZ);
    }
    ...
    {
    	int iRange = getClampedViewDistanceInChunks() - 2;
    ...
    {
        Iterator<ChunkCoordIntPair> tempIterator = activeChunksCoordsList.iterator();
        ...
        while ( tempIterator.hasNext() )
        {
            ...
            tempIterator.remove();
            activeChunksCoordsMap.remove(lTempKey);
        }
    }
    ...
    protected void addToActiveChunkMap(int iChunkX, int iChunkZ)
    {
        ...
        if ( !activeChunksCoordsMap.containsItem(lCoordKey) )
        {
	    	ChunkCoordIntPair coords = new ChunkCoordIntPair( iChunkX, iChunkZ );
	    	
	        activeChunksCoordsList.add(coords);
	        
	        activeChunksCoordsMap.add(lCoordKey, coords);
        }
    }
    ...
    public boolean isChunkActive(int iChunkX, int iChunkZ)
    {
        ...
        return activeChunksCoordsMap.containsItem(lCoordKey);
    }
