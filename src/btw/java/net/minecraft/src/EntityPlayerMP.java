...
public class EntityPlayerMP extends EntityPlayer implements ICrafting
{
    ...
    public void onUpdateEntity()
    {
        try
        {
        	...
        	if (this.getHealth() != this.lastHealth || this.lastFoodLevel != this.foodStats.getFoodLevel() || this.foodStats.getSaturationLevel() == 0.0F != this.wasHungry ||
                lastFoodSaturation != (int)(foodStats.getSaturationLevel() * 8F ) )
            {
    ...
    public void displayGUIWorkbench(int par1, int par2, int par3)
    {
        ...
        openContainer = new WorkbenchContainer( inventory, worldObj, par1, par2, par3 );
    ...
    // FCMOD: Added
    public LinkedList<ChunkCoordIntPair> chunksToBeSentToClient =
    	new LinkedList<ChunkCoordIntPair>();
    
    private int lastFoodSaturation = -99999999;
	private int exhaustionWithTimeCounter = 0;
	
	private static final int exhaustionWithTimePeriod = 600; // once per 30 seconds
	private static final float exhaustionWithTimeAmount = 0.5F; // set with above to begin starving to death after 160 minutes if you're fully obese, losing 1 hunger every 4 minutes
    
    private static final float minimumGloomBiteChance = 0.01F;
    private static final float maximumGloomBiteChance = 0.05F; // 1/second
    
	private static final int delayBetweenZeroDamageAttackSounds = 20;
	private long timeOfLastZeroDamageAttackSound = 0;
    ...
    protected void checkForHeadDrop(DamageSource source, int iLootingModifier)
    {
	    ...
	    if ( sourceEntity instanceof EntityPlayer)
	    {
	    	if ( ((EntityPlayer)sourceEntity).getHeldItem() != null && 
	    		((EntityPlayer)sourceEntity).getHeldItem().getItem().itemID == BTWItems.battleaxe.itemID )
	    	{
    ...
    private void dropMysteryMeat(int iLootingModifier)
    {
        if ( !hasHeadCrabbedSquid() )
        {
			...
			if (timeOfLastSpawnAssignment == 0 || timeOfLastSpawnAssignment > lOverworldTime ||
                lOverworldTime - timeOfLastSpawnAssignment >= HardcoreSpawnUtils.HARDCORE_SPAWN_TIME_BETWEEN_REASSIGNMENTS)
			{
				int iDropItemID = BTWItems.rawMysteryMeat.itemID;
                
                if (validateBoundRespawnBeacon(this.worldObj, this.dimension, this.spawnDimension).beaconStatus != BeaconStatus.MISSING) {
	...
	private void updateMagneticInfluences()
	{
	    ...
	    if ( worldObj.provider.isSurfaceWorld() )
	    {
	        ...
	        Iterator pointIterator = worldObj.getMagneticPointList().magneticPoints.iterator();
	         
	        while ( pointIterator.hasNext() )
	        {
	        	MagneticPoint tempPoint = (MagneticPoint)pointIterator.next();
	    ...
	    if ( strongestPoint != null )
	    {
	    	...
	    	setStrongestMagneticPointForLocationI(strongestPoint.posX);
	    	setStrongestMagneticPointForLocationK(strongestPoint.posZ);
	    }
	    else
	    {
	...
	private void updateSpawnChunksVisualization()
	{
		if ( worldObj.provider.dimensionId == 0 && (isWearingEnderSpectacles() ||
                                                    isPotionActive( BTWMod.potionTrueSight ) ) )
		{
    ...
    {
    	exhaustionWithTimeCounter++;
    	
    	if (exhaustionWithTimeCounter >= exhaustionWithTimePeriod)
    	{
            if ( !capabilities.disableDamage ) // disable hunger drain in creative
            {
            	foodStats.addExhaustion(exhaustionWithTimeAmount);
            }

            exhaustionWithTimeCounter = 0;
    	}
    }
    
    private void updatePotionEffectsFromStatus() {
        if (!isDead && (worldObj.getTotalWorldTime() + entityId) % 40L == 0L && !this.capabilities.isCreativeMode) {
    ...
    protected void updateGloomState() {
        if (!isDead) {
            if (isInGloom() && !this.capabilities.isCreativeMode) {
                ...
                if (getGloomLevel() == 0 || (inGloomCounter > GLOOM_COUNTER_BETWEEN_STATE_CHANGES && getGloomLevel() < 3)) {
                ...
                if (getGloomLevel() >= 3) {
                    ...
                    if (rand.nextFloat() < gloomBiteChance) {
                        if (attackEntityFrom(CustomDamageSource.damageSourceGloom, 1)) {
                            if (health <= 0) {
                                worldObj.playAuxSFX(BTWEffectManager.BURP_SOUND_EFFECT_ID, MathHelper.floor_double(posX), MathHelper.floor_double(posY),
    ...
    protected void onZeroDamageAttack()
    {
		...
		if (lCurrentTime > timeOfLastZeroDamageAttackSound + delayBetweenZeroDamageAttackSounds)
		{
			worldObj.playSoundAtEntity( this, 
	    		"random.classic_hurt", 1.0F +  rand.nextFloat() * 0.25F, 
	    		getSoundPitch() * 1.20F + rand.nextFloat() * 0.1F);

            timeOfLastZeroDamageAttackSound = lCurrentTime;
		}
    }
    ...
    {
        if ( !chunksToBeSentToClient.isEmpty() )
        {
            Iterator<ChunkCoordIntPair> coordIterator = chunksToBeSentToClient.iterator();
            ...
            {
	        	WorldUtils.sendPacketToPlayer(playerNetServerHandler,
