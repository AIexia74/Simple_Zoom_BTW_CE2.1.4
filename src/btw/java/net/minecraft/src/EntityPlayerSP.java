...
public class EntityPlayerSP extends EntityPlayer
{
    ...
    public void onLivingUpdate()
    {
        ...
        else
        {
            ...
            this.pushOutOfBlocks(this.posX + (double)this.width * 0.35D, this.boundingBox.minY + 0.5D, this.posZ - (double)this.width * 0.35D);
            this.pushOutOfBlocks(this.posX + (double)this.width * 0.35D, this.boundingBox.minY + 0.5D, this.posZ + (double)this.width * 0.35D);
            boolean var4 = !doesStatusPreventSprinting() || this.capabilities.allowFlying;
    ...
    public float getFOVMultiplier()
    {
        ...
        if ( isUsingItem() && ( getItemInUse().itemID == Item.bow.itemID ||
    		getItemInUse().itemID == BTWItems.compositeBow.itemID ) )
        {
    ...
    private static final float MINIMUM_GLOOM_CAVE_SOUND_CHANCE = 0.01F; // 1/5 seconds
    private static final float MAXIMUM_GLOOM_CAVE_SOUND_CHANCE = 0.05F; // 1/second
    ...
    private static final float MINIMUM_GLOOM_GROWL_SOUND_CHANCE = 0.01F; // 1/5 seconds
    private static final float MAXIMUM_GLOOM_GROWL_SOUND_CHANCE = 0.05F; // 1/second
    ...
    private static final float GLOOM_FOV_MULTIPLIER_TIME_FOR_TRANSITION_IN = 10.0F; // in seconds
    private static final float GLOOM_FOV_MULTIPLIER_TIME_FOR_TRANSITION_OUT = 2.0F; // in seconds
    private static final float GLOOM_FOV_MULTIPLIER_DELTA_IN_PER_TICK = ((MAXIMUM_GLOOM_FOV_MULTIPLIER - 1.0F ) / 20F ) / GLOOM_FOV_MULTIPLIER_TIME_FOR_TRANSITION_IN;
    private static final float GLOOM_FOV_MULTIPLIER_DELTA_OUT_PER_TICK = ((MAXIMUM_GLOOM_FOV_MULTIPLIER - 1.0F ) / 20F ) /
                                                                         GLOOM_FOV_MULTIPLIER_TIME_FOR_TRANSITION_OUT;
    
    private float currentGloomFOVMultiplier = 1F;
    ...
    public void addExhaustion( float fAmount )
    {
        ...
        {
            exhaustionAddedSinceLastGuiUpdate = true;
        }
    ...
    protected void updateGloomState()
    {
		...
		if (previousGloomLevel != getGloomLevel() )
		{
            inGloomCounter = 0;
            previousGloomLevel = iGloomLevel;
    	...
    	if ( iGloomLevel > 0 )
    	{
    		...
    		float fCounterProgress = (float) inGloomCounter / (float) GLOOM_COUNTER_BETWEEN_STATE_CHANGES;
