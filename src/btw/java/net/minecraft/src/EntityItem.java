...
public class EntityItem extends Entity
{
    ...
    public boolean attackEntityFrom(DamageSource par1DamageSource, int par2)
    {
        ...
        else
        {
	        ...
	        // FCMOD: Code added
	        if ( !worldObj.isRemote && !isDead && getEntityItem().getItem().itemID == BTWItems.blastingOil.itemID )
	        {
	    		detonateBlastingOil();
    ...
    public void writeEntityToNBT(NBTTagCompound par1NBTTagCompound)
    {
        ...
        // FCMOD: Code added
        par1NBTTagCompound.setLong("fcDespawnTime", absoluteItemDespawnTime);
	    // END FCMOD    
    ...
    public void readEntityFromNBT(NBTTagCompound par1NBTTagCompound)
    {
	    ...
	    {
            absoluteItemDespawnTime = par1NBTTagCompound.getLong("fcDespawnTime");
	    }
	    // END FCMOD	   
    ...
    // FCMOD: Code added    
    private long absoluteItemDespawnTime = 0;
    ...
    {
        if ( BTWMod.isHardcoreBuoyEnabled( worldObj ) )
        {
    ...
    protected void fall( float fFallDistance )
    {
        ...
        if ( !worldObj.isRemote )
        {
	        if ( getEntityItem().getItem().itemID == BTWItems.blastingOil.itemID )
	        {
	        	if ( fFallDistance > 3F )
	        	{
	        		detonateBlastingOil();
	...
	private void checkForItemDespawn() {
		if (!worldObj.isRemote) {
			if (droppingPlayerDeathsOnDrop > -1) {
				...
				if (MinecraftServer.getServer().worldServers[0].getTotalWorldTime() >= absoluteItemDespawnTime && deathsBeforeDisappear >= 0) {
			...
			else if (absoluteItemDespawnTime > 0) {
				...
				if (lOverworldTime >= absoluteItemDespawnTime) {
					setDead();
				}
			}
			else {
	...
	public void setEntityItemAsDroppedOnPlayerDeath(EntityPlayer player) {
		if (!worldObj.isRemote) {
			...
			long maxGraceTime = HardcoreSpawnUtils.HARDCORE_SPAWN_TIME_BETWEEN_REASSIGNMENTS;
            ...
            absoluteItemDespawnTime = overworldTime + maxGraceTime;
