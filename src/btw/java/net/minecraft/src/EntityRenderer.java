...
public class EntityRenderer
{
    ...
    private void setupCameraTransform(float par1, int par2)
    {
        ...
        if (var4 > 0.0F)
        {
            ...
            if (intensity < 100F) { // nauseaStrength 160F == config int 0
    ...
    private void updateLightmap(float par1)
    {
        ...
        else if (var2 != null)
        {
            for (int var3 = 0; var3 < 256; ++var3)
            {
                ...
                {
                    var16 = this.previousWithEffectIntensity + (this.witherEffectIntensity - this.previousWithEffectIntensity) * par1;
    ...
    private void updateFogColor(float par1)
    {
        ...
        if (this.witherEffectIntensity > 0.0F)
        {
            var23 = this.previousWithEffectIntensity + (this.witherEffectIntensity - this.previousWithEffectIntensity) * par1;
    ...
    private float previousSunBrightness = -1F;
	...
	private void modUpdateLightmapOverworld(WorldClient world, float fPartialTicks)
	{
    	...
    	if (previousSunBrightness > -0.01F )
    	{
    		if (Math.abs(fCurrentSunBrightness - previousSunBrightness) > maximumSunBrightnessDelta)
    		{
    			if (fCurrentSunBrightness > previousSunBrightness)
    			{
    				fCurrentSunBrightness = previousSunBrightness + maximumSunBrightnessDelta;
    			}
    			else
    			{
    				fCurrentSunBrightness = previousSunBrightness - maximumSunBrightnessDelta;
        ...
        previousSunBrightness = fCurrentSunBrightness;
        ...
        for ( int iTempMapIndex = 0; iTempMapIndex < 256; ++iTempMapIndex )
        {
            ...
            if (witherEffectIntensity > 0.0F )
            {
                ...
                float fCurrentWitherEffectIntensity = previousWithEffectIntensity + (witherEffectIntensity - previousWithEffectIntensity) * fPartialTicks;
