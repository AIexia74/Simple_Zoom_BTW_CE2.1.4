        ...
        else
        {
            if (!this.worldObj.isRemote)
            {
                ...
                if (angler.isDead || !angler.isEntityAlive() || var1 == null ||
                    !(( var1.getItem() == Item.fishingRod && !isBaited) ||
                      ( var1.getItem() == BTWItems.baitedFishingRod && isBaited) ) ||
            ...
            if (!this.inGround)
            {
                ...
                for (int var29 = 0; var29 < var27; ++var29)
                {
                    ...
                    if (this.worldObj.isAABBInMaterial(var18, Material.water))
                    {
                        ...
                        //Bubbles to indicate valid fishing spot
                        if (rand.nextInt(10) == 1) {
                            if (isBodyOfWaterLargeEnoughForFishing()) {
                                float y = (float) MathHelper.floor_double(this.boundingBox.minY);
                                float x = (this.rand.nextFloat() * 2.0F - 1.0F);
                                float z = (this.rand.nextFloat() * 2.0F - 1.0F);
                                this.worldObj.spawnParticle("bubble", this.posX + x, y + 1.0F, this.posZ + z, 0, 0, 0);
                            }
                        }
                    }
                }
        ...
        for (int i = x - maxRadius; i <= x + maxRadius; i++) {
            for (int j = y - maxRadius; j <= y; j++) {
                for (int k = z - maxRadius; k <= z + maxRadius; k++) {
                    if (!WorldUtils.isWaterSourceBlock(worldObj, i, j, k)
