...
public class WoolItem extends Item
{
    ...
    static private List<List<Integer>> colorConversionArray = null;
        ...
        for ( int iTempSlot = 0; iTempSlot < inventory.getSizeInventory(); ++iTempSlot )
        {
            ...
            if ( tempStack != null )
            {
            	if ( tempStack.itemID == BTWItems.wool.itemID ||
            		tempStack.itemID == BTWItems.woolKnit.itemID )
            	{
            		int iWoolColorIndex = MathHelper.clamp_int( tempStack.getItemDamage(), 0, 15 );
            		
            		int iWoolColor = WoolItem.woolColors[iWoolColorIndex];
    ...
    {
		colorConversionArray = new ArrayList<List<Integer>>(16 );
    ...
    {
		int iFromColor1 = WoolItem.woolColors[iFromColorIndex1];
		int iFromColor2 = WoolItem.woolColors[iFromColorIndex2];
