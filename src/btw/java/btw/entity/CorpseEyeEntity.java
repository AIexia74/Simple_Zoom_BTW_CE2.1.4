...
public class CorpseEyeEntity extends Entity {
	...
	static final private int VEHICLE_SPAWN_PACKET_TYPE = 120;

	public CorpseEyeEntity(World par1World) {
		super(par1World);
	...
	public CorpseEyeEntity(World par1World, double par2, double par4, double par6) {
		super(par1World);
		this.despawnTimer = 0;
		this.setSize(0.25F, 0.25F);
	...
	protected void checkForDropCoupledItem() {
		if (itemDamage < 9) {
			ItemStack stack = new ItemStack(BTWItems.corpseEye);
			...
			this.worldObj.playAuxSFX(BTWEffectManager.ANIMAL_BIRTHING_EFFECT_ID, MathHelper.floor_double(posX), MathHelper.floor_double(posY),
