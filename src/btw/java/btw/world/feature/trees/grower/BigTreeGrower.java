...
public class BigTreeGrower extends AbstractTreeGrower {
				...
				for (double var9 = 0.5D; var7 < var1; ++var7) {
					...
					if (this.checkBlockLine(var17, var18) == -1) {
						...
						double var20 = Math.sqrt(
								Math.pow((double) Math.abs(this.basePos[0] - var17[0]), 2.0D) + Math.pow((double) Math.abs(this.basePos[2] - var17[2]), 2.0D));
	...
	public void generateLeafNode(int par1, int par2, int par3, boolean isWorldGen) {
		...
		for (int var5 = par2 + this.leafDistanceLimit; var4 < var5; ++var4) {
			...
			this.genTreeLayer(par1, var4, par3, var6, (byte) 1, this.woodType.leavesBlockID, this.woodType.leavesMetadata, isWorldGen);
		}
	}
