...
public class MiscUtils
{
    ...
    static private void positionEntityOutsideOfLocation(World world, Entity entity, int i, int j, int k)
    {
		...
		if ( entity instanceof EntityPlayerMP )
		{
			...
			WorldUtils.sendPacketToPlayer(player.playerNetServerHandler,
    ...
    static public boolean standardRotateAroundY(Block block, World world, int i, int j, int k, boolean bReverse)
    {
	...
	static public int standardRotateMetadataAroundY(Block block, int iMetadata, boolean bReverse)
	{
	 ...
	 */ 
	static public void placeNonPersistentWater(World world, int i, int j, int k)
	{
	...
	static public void placeNonPersistentWaterMinorSpread(World world, int i, int j, int k)
	{
		int iSpread = 5;
    ...
    static private boolean doesWaterHaveValidSourceRecursive(World world, int i, int j, int k, int startI, int startJ, int startK, int iDistanceToCheck)
    {
    	...
    	for ( int iFacing = 2; iFacing < 6; iFacing++ )
    	{
			...
			else if ( BTWBlocks.potentialFluidSources[iTargetBlockID] )
			{
				...
				if (targetBlock.isSourceToFluidBlockAtFacing(world, targetPos.x, targetPos.y, targetPos.z, Block.getOppositeFacing(iFacing)) >= 0 )
