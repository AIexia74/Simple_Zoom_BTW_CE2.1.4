...
public class RayTraceUtils
{
	...
	List<MovingObjectPosition> intersectionPointList;
	
	public RayTraceUtils(World world, int i, int j, int k, Vec3 startRay, Vec3 endRay )
	{
        intersectionPointList = new ArrayList<MovingObjectPosition>();
	...
	{
        Vec3 startRay = this.startRay.addVector(-blockX, -blockY, -blockZ);
        Vec3 endRay = this.endRay.addVector(-blockX, -blockY, -blockZ);
        ...
        if (vec3_6 != null)
        {
			...
			intersectionPointList.add(new MovingObjectPosition(blockX, blockY, blockZ, byte0, vec3_6.addVector(blockX, blockY, blockZ) ));
        }
	}
	...
	public void addQuadWithLocalCoordsToIntersectionList(PrimitiveQuad quad, Vec3 pointOnQuad)
	{
        ...
        Vec3 lineDir = endRay.subtractFrom(startRay);
        
        Vec3 pointOnLine = startRay.addVector(-blockX, -blockY, -blockZ);
        ...
        if ( lDotn != 0D ) // dot product of zero means ray and plane are parallel, with zero intersections
        {
            ...
            if ( dNumeratorDot != 0D ) // dot product of zero means line is within plane, with infinite intersections
            {
            	...
            	if ( quad.isPointOnPlaneWithinBounds(intersection) )
            	{
	    			...
	    			intersectionPointList.add(new MovingObjectPosition(blockX, blockY, blockZ,
                                                                       iFacingClicked,
                                                                       intersection.addVector(blockX, blockY, blockZ) ));
            	}
            }
        }
	}
	...
	public MovingObjectPosition getFirstIntersection()
	{
        ...
        Iterator iterator = intersectionPointList.iterator();
        ...
        while ( iterator.hasNext() )
        {
            ...
            double dCurrentIntersectDistance = tempPos.hitVec.squareDistanceTo(endRay);
