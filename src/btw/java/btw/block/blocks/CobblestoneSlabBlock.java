...
public class CobblestoneSlabBlock extends BlockHalfSlab {
    ...
    protected ItemStack createStackedBlock( int iMetadata ) {
        return new ItemStack( BTWBlocks.cobblestoneSlab.blockID, 2, getStrata(iMetadata) );
    }

	@Override
	public boolean dropComponentItemsOnBadBreak(World world, int i, int j, int k, int iMetadata, float fChanceOfDrop) {
		int numItems = isDoubleSlab ? 2 : 1;
		dropItemsIndividually(world, i, j, k, BTWBlocks.looseCobblestoneSlab.blockID, numItems, getStrata(iMetadata) << 2, fChanceOfDrop);
