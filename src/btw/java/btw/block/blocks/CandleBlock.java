...
public class CandleBlock extends Block {
    ...
    public boolean canPlaceBlockAt(World world, int x, int y, int z) {
		...
		if (blockBelowID == BTWBlocks.aestheticNonOpaque.blockID && blockBelowMetadata == AestheticNonOpaqueBlock.SUBTYPE_LIGHTNING_ROD) {
			return true;
		}

		return WorldUtils.doesBlockHaveSmallCenterHardpointToFacing(world, x, y - 1, z, 1, true);
    }
    ...
    public void dropBlockAsItemWithChance(World world, int x, int y, int z, int metadata, float chance, int fortuneModifier) {
        if (!world.isRemote) {
    		this.dropItemsIndividually(world, x, y, z,
	...
	public boolean convertBlock(ItemStack stack, World world, int x, int y, int z, int fromSide) {
		...
		this.dropItemsIndividually(world, x, y, z, this.idDropped(metadata, world.rand, 0), 1, this.damageDropped(metadata), 1);
	...
	public boolean setOnFireDirectly(World world, int x, int y, int z) {
		if (!this.isLit(world, x, y, z)) {
			...
			world.playSoundEffect( x + 0.5D, y + 0.5D, z + 0.5D, "mob.ghast.fireball",
